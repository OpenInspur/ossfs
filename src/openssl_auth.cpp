...
static pthread_mutex_t* ossfs_crypt_mutex = NULL;

static void ossfs_crypt_mutex_lock(int mode, int pos, const char* file, int line)
{
  if(ossfs_crypt_mutex){
    if(mode & CRYPTO_LOCK){
      pthread_mutex_lock(&ossfs_crypt_mutex[pos]);
    }else{
      pthread_mutex_unlock(&ossfs_crypt_mutex[pos]);
    }
  }
}

static unsigned long ossfs_crypt_get_threadid()
{
...
static struct CRYPTO_dynlock_value* ossfs_dyn_crypt_mutex(const char* file, int line)
{
  ...
  if(NULL == (dyndata = static_cast<struct CRYPTO_dynlock_value*>(malloc(sizeof(struct CRYPTO_dynlock_value))))){
    OSSFS_PRN_CRIT("Could not allocate memory for CRYPTO_dynlock_value");
    return NULL;
  }
...
static void ossfs_dyn_crypt_mutex_lock(int mode, struct CRYPTO_dynlock_value* dyndata, const char* file, int line)
{
  if(dyndata){
    if(mode & CRYPTO_LOCK){
...
static void ossfs_destroy_dyn_crypt_mutex(struct CRYPTO_dynlock_value* dyndata, const char* file, int line)
{
  if(dyndata){
...
{
  if(ossfs_crypt_mutex){
    OSSFS_PRN_DBG("ossfs_crypt_mutex is not NULL, destroy it.");
    if(!ossfs_destroy_crypt_mutex()){
      OSSFS_PRN_ERR("Failed to ossfs_crypt_mutex");
      return false;
    }
  }
  if(NULL == (ossfs_crypt_mutex = static_cast<pthread_mutex_t*>(malloc(CRYPTO_num_locks() * sizeof(pthread_mutex_t))))){
    OSSFS_PRN_CRIT("Could not allocate memory for ossfs_crypt_mutex");
    return false;
  }
  for(int cnt = 0; cnt < CRYPTO_num_locks(); cnt++){
    pthread_mutex_init(&ossfs_crypt_mutex[cnt], NULL);
  }
  // static lock
  CRYPTO_set_locking_callback(ossfs_crypt_mutex_lock);
  CRYPTO_set_id_callback(ossfs_crypt_get_threadid);
  // dynamic lock
  CRYPTO_set_dynlock_create_callback(ossfs_dyn_crypt_mutex);
  CRYPTO_set_dynlock_lock_callback(ossfs_dyn_crypt_mutex_lock);
  CRYPTO_set_dynlock_destroy_callback(ossfs_destroy_dyn_crypt_mutex);
...
bool ossfs_destroy_crypt_mutex()
{
  ...
  for(int cnt = 0; cnt < CRYPTO_num_locks(); cnt++){
    pthread_mutex_destroy(&ossfs_crypt_mutex[cnt]);
  }
  CRYPTO_cleanup_all_ex_data();
  free(ossfs_crypt_mutex);
  ossfs_crypt_mutex = NULL;
